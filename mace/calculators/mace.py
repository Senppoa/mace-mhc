...
class MACECalculator(Calculator):
    ...
    args:
        ...
        head_committee: list[str] | bool, treat a set of output heads of a *single*
                    model as a committee for uncertainty quantification (multi-head
                    committee, MHC). Pass an explicit list of head names, or ``True``
                    to auto-select every head whose name contains ``"committee"``.
                    When active, the calculator runs one forward pass per committee
                    head (sharing the message-passing backbone) and exposes the mean
                    prediction plus committee spread via the same ``*_comm`` / ``*_var``
                    result keys used by a multi-model committee. See
                    Beck et al., J. Chem. Phys. 163, 234103 (2025).
    ...
    def __init__(
        ...
        pad_num_edges: int = 0,
        warmup: bool = False,
        head_committee: Union[List[str], bool, None] = None,
        **kwargs,
    ):
        ...
        # Multi-head committee (MHC): a set of heads of a single model treated as
        # a committee for uncertainty quantification. Resolve the member heads here
        # so the (possibly missing) `head` kwarg logic below can fall back to a
        # committee member instead of erroring on a missing "default" head.
        self.head_committee: Union[List[str], None] = self._resolve_head_committee(
            head_committee
        )

        kwarg_head = kwargs.get("head", None)
        if kwarg_head is None and self.head_committee is not None:
            # No explicit head requested: use the first committee member as the
            # nominal single head (predictions are averaged over all members anyway).
            kwarg_head = self.head_committee[0]
        ...
        if self.head_committee is not None:
            logging.info(
                f"Running multi-head committee with {len(self.head_committee)} "
    ...
    def _resolve_head_committee(self, head_committee):
        ...
        if head_committee is True:
            ...
            if not members:
                raise ValueError(
                    "head_committee=True but no head name contains 'committee'. "
                    f"Available heads: {self.available_heads}. Pass an explicit "
                    "list of head names instead."
                )
        else:
            members = list(head_committee)
            missing = [h for h in members if h not in self.available_heads]
            if missing:
                raise ValueError(
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        # Build the list of committee passes.
        # - Multi-model committee (default): one pass per model, no head override.
        # - Multi-head committee (MHC): one pass per committee head, all sharing
        #   self.models[0] (the message-passing backbone is evaluated once per head).
        if self.head_committee is not None:
            passes = [
                (self.models[0], self.available_heads.index(h))
                for h in self.head_committee
            ]
        else:
            passes = [(model, None) for model in self.models]
        num_members = len(passes)

        ret_tensors = None
        node_e0 = None
        for i, (model, head_idx) in enumerate(passes):
            batch = self._clone_batch(batch_base)
            if head_idx is not None:
                # Override the per-graph head index so this pass uses committee head i.
                batch["head"] = torch.full_like(batch["head"], head_idx)
            ...
            if i == 0:
                ret_tensors, node_e0 = self._create_result_tensors(
                    num_members, num_real_atoms, batch, out
                )
        ...
        for results_key, ret_key, unit_conv in results_map:
            if ret_tensors.get(ret_key) is not None:
                ...
                if num_members > 1 and results_key in results_store_ensemble:
